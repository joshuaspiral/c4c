...
int main() {
    ...
    SDL_Window *window = SDL_CreateWindow("Connect Four", 100, 100, SCREEN_WIDTH,
        ...
        while (SDL_PollEvent(&e)) {
            switch (e.type) {
                ...
                case SDL_MOUSEBUTTONDOWN:
                    if (!gameover) {
                        if (currPlayer == HUMAN) {
                            if (dropPiece(&boards.yellow, &boards.red,
                                        e.button.x / (SCREENBOARD_WIDTH / WIDTH) - 1, HUMAN,
                                        heights) == 1) {
                                break;
                            }
                        }
                ...
                default: {
                             if (currPlayer == AI) {
                                 dropPiece(&boards.yellow, &boards.red,
                                         aiMove(&boards.yellow, &boards.red, 7, heights), AI,
                                         heights);
                                 currPlayer = ((currPlayer == YELLOW) ? RED : YELLOW);
                             }
                         }
